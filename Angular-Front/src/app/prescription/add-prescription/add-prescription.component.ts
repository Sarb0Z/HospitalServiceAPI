...
export class AddPrescriptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.medicineForm = this.form.group({
      medicine_name: this.form.array([this.form.group({name:this.medicineList})])
    })
  ...
  AddPrescription(){
    let obj = this.medicineList.find((o: { medicine_name: any; }) => o.medicine_name === this.patientDetail.value.medicine);
    ...
    this.service.addPrescription(val).subscribe((res)=>{
      alert(res.toString());
